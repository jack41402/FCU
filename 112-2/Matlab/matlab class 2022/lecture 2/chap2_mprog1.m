...
v	= randn(3,2)			% rand matrix (see also randn)
v1=floor(6.*rand(3,2))
save matrix_data1 m v x      % save the variable m to a file named matrix_data.mat
...
theta=50:10:80;
theta1=theta'
matrix_t=repmat(theta1,1,6);
matrix_v=repmat(v1,4,1);
h=matrix_v.^2.*(sind(matrix_t).^2)./(2*9.8);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
max(a(1,:))		     		% another way to obtain max of matrix
...
%%  Prog 2.6 format , % disp statement % format print
...
disp([ a  b b ])
i=11;
disp(['this is a sample ','"from class": ',num2str(i)])

%% please check "fprintf" from matlab help 

format   compact % (1) short e ; (2) bank (3) compact
x=[ 1e3 1 1e-4]
