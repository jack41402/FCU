...
format long
% x(row,col)=... row, col : the row & column address.
...
a = [1:3; 4:6; 7:9] % to create a 3*3 matrix using colon (:)
b = [11:14; 15:18; -4:-1] % 3*4 matrix
a(:,[1 3]) = b(:,[4 2]) % using [ ] inside ( ) create the address of the matrix
...
a = zeros(3,4)
a = ones(3,4)
a = rand(3,4) % check randn function
a = eye(3)  % elementary matrix
...
% Exercise 2: Do the  methods 3 for the Ex. 2-28 in p. 81 for L=10000:10000:50000; and r=0.1:0.02:0.2, P=1000
% Do the method 1 & 2 as the HW
...
% with a(:,:,1)=[1:2; 3:4].
% For example you want to assign a matric to store an color image 
% R(x,y), G(x,y), B(x,y)
a(:,:,2) = [5:6; 7:8]
...
% 6.6.2 The residual: this is actually a unique solution
r = A*z - b
...
b = [2 1]'
x = A \ b % x is only one of the solution, there exist infinie many solution
...
% perturbed b vector 
% ill-conditions means that the small change in the coefficients leads to a
...
%     near EPS. it looks like the ratio of min(eig(A))/max(eig(A))
format long
rcond(A)
