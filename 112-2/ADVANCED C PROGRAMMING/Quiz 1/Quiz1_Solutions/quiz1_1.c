...
int strncmp_rec(const char *str1, const char *str2, size_t n) {
	if (n<=0) return 0; // If size is zeor, returns 0. 
	else { // Otherwise, perform string comparison.
		if (*str1==*str2) // If two characters are identical,
			if (*str1=='\0') return 0; // If the character is the null character, rerturn 0 for identical strings.
			else return strncmp_rec(str1+1, str2+1, n-1); // Recursively call strcmp_rec with the left of strings.
			// The first pair of disticnt characters is found, 
			else if (*str1>*str2) return 1; // str1 is "greater" than str2.
			else if (*str1<*str2) return -1; // str1 is "less" than str2. 
	}
}
