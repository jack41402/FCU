...
int main() {
	...
	*/ 
	...
	if (orientation){
		fillings_merged = (4 - ((header.Width + 30) * 3) % 4) % 4;
		rowSize_merged = (header.Width + 30) * 3 + fillings_merged;
		imageSize_merged = (header.Height * 2 + 45) * rowSize_merged;
		imageData_merged = (unsigned char*) malloc(imageSize_merged);
		
		for (i = 0; i < (header.Height * 2 + 45); i++){
			for (j = 0; j < (header.Width + 30); j++){
		...
		for (i = 0; i < header.Height; i++){
			for (j = 0; j < header.Width; j++){
				k = i * rowSize + j * 3;
				k1 = (i + 30 + header.Height) * rowSize_merged + (j + 15) * 3;
				k2 = (15 + header.Height - i) * rowSize_merged + (j + 15) * 3;
		...
		header.Height = header.Height * 2 + 45;
		header.ImageSize = imageSize_merged;
		printf("\n>>>> File header and image information of the merged image: landscape.bmp\n\n");
		fptr = fopen("landscape.bmp", "wb");
		
		
	}else{
		fillings_merged = (4 - ((header.Width * 2 + 45) * 3) % 4) % 4;
		rowSize_merged = (header.Width * 2 + 45) * 3 + fillings_merged;
		imageSize_merged = (header.Height + 30) * rowSize_merged;
		imageData_merged = (unsigned char*) malloc(imageSize_merged);
		
		for (i = 0; i < (header.Height + 30); i++){
			for (j = 0; j < (header.Width * 2 + 45); j++){
				k = i * rowSize_merged + j * 3;	
				imageData_merged[k] = frame_B;
				imageData_merged[k+1] = frame_G;
				imageData_merged[k+2] = frame_R;
		...
		for (i = 0; i < header.Height; i++){
			for (j = 0; j < header.Width; j++){
				k = i * rowSize + j * 3;
				k1 = (i + 15) * rowSize_merged + (j + 15) * 3;
				k2 = (i + 15) * rowSize_merged + (header.Width * 2 + 30 - j) * 3;
		...
		header.Size = header.Size - header.ImageSize + imageSize_merged;
		header.Width = header.Width * 2 + 45;
	...
	*/
	...
	fwrite(imageData_merged, header.ImageSize, 1, fptr);
