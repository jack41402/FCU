...
int main() {
	...
	*/ 
	...
	/*****  Complete this code segment.  *****/
	/*****************************************/
	width_merged = header.Width;
	height_merged = header.Height;
	
	imageSize_merged = ceil((float) width_merged*3.0/4.0)*4*height_merged;
	
	imageData_merged = (unsigned char *) malloc(imageSize_merged);
	
	rowSize = ceil((float) header.Width*3.0/4.0)*4;
	
	rowSize_merged = ceil((float) header.Width*3.0/4.0)*4;
	for(i=0;i<height_merged;i++){
		for(j=0;j<width_merged;j++){
			k1 = (height_merged+frame_width*2+i)*rowSize_merged+(frame_width+j)*3;
			k2 = (frame_width*2+i)*rowSize_merged+(frame_width+j)*3;
			imageData_merged[k1] = (unsigned char ) frame_B;
			imageData_merged[k1+1] = (unsigned char ) frame_G;
			imageData_merged[k1+2] = (unsigned char ) frame_R;
			imageData_merged[k2] = (unsigned char ) frame_B;
			imageData_merged[k2+1] = (unsigned char ) frame_G;
			imageData_merged[k2+2] = (unsigned char ) frame_R;
	...
	*/
	...
	fwrite(&header.Type,2,1,fptr);
  	fwrite(&header.Size,1,4,fptr);
  	fwrite(&header.Reserved,4,1,fptr);
  	fwrite(&header.OffsetBits,1,4,fptr);
  	fwrite(&header.InfoSize,1,4,fptr);
  	fwrite(&header.Width,1,4,fptr);
  	fwrite(&header.Height,1,4,fptr);
  	fwrite(&header.Planes,1,2,fptr);
  	fwrite(&header.BitPerPixel,1,2,fptr);
  	fwrite(&header.Compression,1,4,fptr);
  	fwrite(&imageSize_merged,1,4,fptr);
  	fwrite(&header.XResolution,1,4,fptr);
  	fwrite(&header.YResolution,4,1,fptr);
  	fwrite(&header.Colors,1,4,fptr);
  	fwrite(&header.ImportantColors,1,4,fptr);
