...
int main() {
	...
	/*****************************************/  
	fread(Type[2], 2, 1, char Type[2]);
	fread(Size, 4, 1, unsigned Size);
	fread(Reserved[4], 1, 4, char Reserved[4]);
	fread(OffSetbits, 4, 1, unsigned OffSetbits);
	fread(InfoSize, 4, 1, unsigned InfoSize);
	fread(Width, 4, 1, unsigned Width);
	fread(Height, 4, 1, unsigned Height);
	fread(Planes, 4, 1, unsigned short Planes);
	fread(BitPerPixel, 4, 1, unsigned short BitPerPixel);
	fread(Compression, 4, 1, unsigned compression);
	fread(ImageSize, 4, 1, unsigned ImageSize);
	fread(XResolution, 4, 1, unsigned XResolution);
	fread(YResolution, 4, 1, unsigned YResolution);
	fread(Colors, 4, 1, unsigned Colors);
	fread(ImportantColors, 4, 1, unsigned ImportantColors);
	...
	*/
	...
	/*****  Complete this code segment.  *****/
	/*****************************************/  
	fwrite(Type[2], 2, 1, char Type[2]);
	fwrite(Size, 4, 1, unsigned Size);
	fwrite(Reserved[4], 1, 4, char Reserved[4]);
	fwrite(OffSetbits, 4, 1, unsigned OffSetbits);
	fwrite(InfoSize, 4, 1, unsigned InfoSize);
	fwrite(Width, 4, 1, unsigned Width);
	fwrite(Height, 4, 1, unsigned Height);
	fwrite(Planes, 4, 1, unsigned short Planes);
	fwrite(BitPerPixel, 4, 1, unsigned short BitPerPixel);
	fwrite(Compression, 4, 1, unsigned compression);
	fwrite(ImageSize, 4, 1, unsigned ImageSize);
	fwrite(XResolution, 4, 1, unsigned XResolution);
	fwrite(YResolution, 4, 1, unsigned YResolution);
	fwrite(Colors, 4, 1, unsigned Colors);
	fwrite(ImportantColors, 4, 1, unsigned ImportantColors);
