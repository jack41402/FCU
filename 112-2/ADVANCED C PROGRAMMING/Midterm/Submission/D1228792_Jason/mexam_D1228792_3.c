...
int main() {
	...
	fread(palette,4,header.OffsetBits - header.InfoSize - 14,fptr);
	...
	*/ 
	...
	if(orientation == 0){
		...
		width_merged = header.Width * 2 + 3 * 15;
		height_merged = header.Height + 2 * 15;
		fillings = (4 - (width_merged * 3) % 4) % 4;
		rowSize_merged = width_merged * 3 + fillings;
		imageSize_merged = height_merged * rowSize_merged;
		...
		palette = (unsigned char *) malloc(header.OffsetBits - header.InfoSize - 14);
		fwrite(palette,4,header.OffsetBits - header.InfoSize - 14,fptr);
		imageData_merged = (unsigned char *) malloc(imageSize_merged);
		for(i=0;i<height_merged;i++){
			for(j=0;j<width_merged;j++){
				if(i<15||i>=header.Height+15&&i<header.Height+30){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+2] = frame_R;
				}
				if(j<15||j>=header.Width+15&&j<header.Width+30||j>=header.Width*2+30&&j<header.Width*2+45){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+2] = frame_R;
				}
				if(j>=15&&j<header.Width+15&&i>=15&&i<header.Height+15){
					k = (i-15) * rowSize + (j-15) * 3;
				...
				if(j>=header.Width+30&&j<header.Width*2+30&&i>=15&&i<header.Height+15){
					k = (i-15) * rowSize + (j-header.Width-30) * 3;
					k2 = i * rowSize_merged + (j +header.Width-(j-30-header.Width)*2) * 3;
		...
		width_merged = header.Width + 2 * 15;
		height_merged = header.Height * 2 + 3 * 15;
		fillings = (4 - (width_merged * 3) % 4) % 4;
		rowSize_merged = width_merged * 3 + fillings;
		imageSize_merged = height_merged * rowSize_merged;
		header.Size = imageSize_merged + header.OffsetBits;
		...
		for(i=0;i<height_merged;i++){
			for(j=0;j<width_merged;j++){
				if(i<15||i>=header.Height+15&&i<header.Height+30||i>=header.Height*2+30&&i<header.Height*2+45){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+2] = frame_R;
				}
				if(j<15||j>=header.Width+15&&j<header.Width+30){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+2] = frame_R;
				}
				if(i>=15&&i<header.Height+15&&j>=15&&j<header.Width+15){
					k = (i-15) * rowSize + (j-15) * 3;
					k1 = (i+header.Height-(i-15)*2) * rowSize_merged + j * 3;
					imageData_merged[k1] = imageData[k];
					imageData_merged[k1+1] = imageData[k+1];
					imageData_merged[k1+2] = imageData[k+2];
				}
				if(i>=header.Height+30&&i<header.Height*2+30&&j>=15&&j<header.Width+15){
					k = (i-header.Height-30) * rowSize + (j-15) * 3;
					k2 = i * rowSize_merged + j * 3;
					imageData_merged[k2] = imageData[k];
					imageData_merged[k2+1] = imageData[k+1];
					imageData_merged[k2+2] = imageData[k+2];
				}
	...
	*/
	...
	printf("ImageSize:        %u\n", imageSize_merged); // Size of the actual pixel data, in bytes.
