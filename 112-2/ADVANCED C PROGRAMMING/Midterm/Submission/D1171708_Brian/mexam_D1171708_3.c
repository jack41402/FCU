...
int main() {
	...
	/*****************************************/ 
	fread(header.Type[0],fptr);
	fread(header.Type[1],fptr);
	fread(header.Reserved[0], header.Reserved[1], header.Reserved[2], header.Reserved[3],1,fptr);
	fread(header.ImageSize,1,fptr);
	fread(header.OffsetBits,1,fptr);
	fread(header.InfoSize,1,fptr);
	fread(header.Width,1,fptr);
	fread(header.Height,1,fptr);
	fread(header.Planes,1,fptr);
	fread(header.BitPerPixel,1,fptr);
	fread(header.Size,1,fptr);
	fread(header.ImageSize,1,fptr);
	fread(header.XResolution,1,fptr);
	fread(header.YResolution,1,fptr);
	fread(header.ImportantColors,1,fptr);
	...
	if(orientation==1){
		printf(">>>> File header and image information of the merged image: landscape.bmp");
		height_merged = ceil((float)(header.Height*2)+3*frame_width);
		width_merged = ceil((float)(header.Width)+2*frame_width);
		imageSize_merged = ceil((width_merged*3/4)*4 * height_merged);
		imageData_merged = (unsigned char)malloc(imageSize_merged);
		fillings = ((4-header.Width%4))%4;
		rowSize = header.Width + fillings;
		fillings = ((4-width_merged%4))%4;
		rowSize_merged = width_merged + fillings;
		for(i=0;i<rowSize_merged;i++){
		...
		k = 0;
		k1 = (rowSize_merged*3)*((height_merged+frame_width)/2)+frame_width*3;
		k2 = (rowSize_merged*3)*frame_width+frame_width*3;
		for(i=0;i<rowSize;i++){
	...
	if(orientation==0){
		printf(">>>> File header and image information of the merged image: portrait.bmp");
		height_merged = ceil((float)(header.Height)+2*frame_width);
		width_merged = ceil((float)(header.Width*2)+3*frame_width);	
		imageSize_merged = ceil((width_merged*3/4)*4 * height_merged);	
		imageData_merged = (unsigned char)malloc(imageSize_merged);
		fillings = ((4-header.Width%4))%4;
		rowSize = header.Width + fillings;
		...
		k = 0;
		k1 = ((rowSize_merged)*3)*(height_merged)+frame_width*3;
		k2 = ((rowSize_merged)*3)*(height_merged)+((rowSize_merged+frame_width)/2)*3;
		for(i=0;i<rowSize;i++){
			for(j=0;j<header.Height;j++){
				imageData_merged[k1] = imageData[k];
				imageData_merged[k1+1] = imageData[k+1];
	...
	header.InfoSize = imageSize_merged+header.OffsetBits;
	...
	*/
	...
	write(header.Type[0]);
	write(header.Type[1]);
	write(header.Reserved[0], header.Reserved[1], header.Reserved[2], header.Reserved[3],1,fptr);
	write(header.Size,1,fptr);
	write(header.OffsetBits,1,fptr);
	write(header.InfoSize,1,fptr);
	write(header.Width,1,fptr);
	write(header.Height,1,fptr);
	write(header.Planes,1,fptr);
	write(header.BitPerPixel,1,fptr);
	write(header.Size,1,fptr);
	write(header.ImageSize,1,fptr);
	write(header.XResolution,1,fptr);
	write(header.YResolution,1,fptr);
	write(header.ImportantColors,1,fptr);
