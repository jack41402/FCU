...
int main() {
	...
	rowSize = header.Width*2*3-fillings;
	width_merged = header.Width*3 + frame_width*3*2;
	height_merged = header.Height + frame_width*3*2;
	imageSize_merged = header.Height*2*3 + header.Width*3;
	...
	*/ 
	...
	for(i = 0; i < header.Height; i++) {
		for(j = 0; j < header.Width; i++) {
			k = header.Height*i + header.Width*j*3;
			k1 = header.Height*i + (header.Width*2-1-j)*j*3;
			k2 = header.Height*i + header.Width*j*3;
			imageData_merged[k1] = imageData_merged[k];
			imageData_merged[k2] = imageData_merged[k];
			imageData_merged[k1+1] = imageData_merged[k+1];
			imageData_merged[k2+1] = imageData_merged[k+1];
			imageData_merged[k1+2] = imageData_merged[k+2];
			imageData_merged[k2+2] = imageData_merged[k+2];
		}
	}
}
	...
	for(i = 0; i < header.Height; i++) {
		for(j = 0; j < header.Width; i++) {
			k = header.Height*i + header.Width*j*3;
			k1 = (header.Height*2-1-i)*i + header.Width*j*3;
			k2 = header.Height*i + header.Width*j*3;
			imageData_merged[k1] = imageData_merged[k];
			imageData_merged[k2] = imageData_merged[k];
			imageData_merged[k1+1] = imageData_merged[k+1];
