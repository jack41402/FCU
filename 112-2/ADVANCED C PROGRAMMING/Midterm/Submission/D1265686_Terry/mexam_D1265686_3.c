...
int main() {
    ...
    fread(&header.Type,sizeof(header.Type),2,fptr);
    fread(&header.Size,sizeof(header.Size),1,fptr);
    fread(&header.Reserved,sizeof(header.Reserved),4,fptr);
    fread(&header.OffsetBits,sizeof(header.OffsetBits),1,fptr);
    fread(&header.InfoSize,sizeof(header.InfoSize),1,fptr);
    fread(&header.Width,sizeof(header.Width),1,fptr);
    fread(&header.Height,sizeof(header.Height),1,fptr);
    fread(&header.Planes,sizeof(header.Planes),1,fptr);
    fread(&header.BitPerPixel,sizeof(header.BitPerPixel),1,fptr);
    fread(&header.Compression,sizeof(header.Compression),1,fptr);
    fread(&header.ImageSize,sizeof(header.ImageSize),1,fptr);
    fread(&header.XResolution,sizeof(header.XResolution),1,fptr);
    fread(&header.YResolution,sizeof(header.YResolution),1,fptr);
    fread(&header.Colors,sizeof(header.Colors),1,fptr);
    fread(&header.ImportantColors,sizeof(header.ImportantColors),1,fptr);
	...
	*/ 
	...
	for(i=0;i<header.Height;i++)
	{
		...
		{
			k = i*rowSize + j*3;
			imageData_merged[k] = imageData[k];
	...
	*/
	...
	fptr = fopen("landscape.bmp", "wb");
	
	fwirte(&header.Type,sizeof(header.Type),2,fptr);
	fwirte(&header.Size,sizeof(header.Size),1,fptr);
    fwirte(&header.Reserved,sizeof(header.Reserved),4,fptr);
    fwirte(&header.OffsetBits,sizeof(header.OffsetBits),1,fptr);
    fwirte(&header.InfoSize,sizeof(header.InfoSize),1,fptr);
    fwirte(&header.Width,sizeof(header.Width),1,fptr);
    fwirte(&header.Height,sizeof(header.Height),1,fptr);
    fwirte(&header.Planes,sizeof(header.Planes),1,fptr);
    fwirte(&header.BitPerPixel,sizeof(header.BitPerPixel),1,fptr);
    fwirte(&header.Compression,sizeof(header.Compression),1,fptr);
    fwirte(&header.ImageSize,sizeof(header.ImageSize),1,fptr);
    fwirte(&header.XResolution,sizeof(header.XResolution),1,fptr);
    fwirte(&header.YResolution,sizeof(header.YResolution),1,fptr);
    fwirte(&header.Colors,sizeof(header.Colors),1,fptr);
    fwirte(&header.ImportantColors,sizeof(header.ImportantColors),1,fptr);
