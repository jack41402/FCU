...
int main() {
	...
	int rowSize, fillings; // Number of bytes in a row. 
	...
	if(orientation){
		width_merged = header.Width + 30;
		height_merged = 2 * header.Height + 45;
		rowSize_merged = width_merged * 3 + (4 - (width_merged * 3) %4)%4;
		imageSize_merged = height_merged*rowSize_merged;
		imageData_merged = (unsigned char*)malloc(imageSize_merged);
		for(i=0; i<header.Height; i++){
			for(j=0; j<header.Width; ++j){
				k = (header.Height - 1 - i) * rowSize + j * 3;
				k1 = (30 + 2*header.Height - 1 - i) * rowSize_merged + (15 + j)* 3;
				k2 = (15 + i) * rowSize_merged + (15 + j) * 3;
		...
		for(i=0; i<height_merged; i++){
			for(j=0; j<header.Width; ++j){
				if(i<=15 || i>=(2*header.Height + 30) || (i>=(header.Height + 15) && i<=(header.Height + 30))){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+1] = frame_R;
				}
			}
		}
	}
	else{
		width_merged = header.Width * 2 + 45;
		height_merged = header.Height + 30;
		rowSize_merged = width_merged * 3 + (4 - (width_merged * 3) %4)%4;
		imageSize_merged = height_merged*rowSize_merged;
		imageData_merged = (unsigned char*)malloc(imageSize_merged);
		for(i=0; i<header.Height; i++){
			for(j=0; j<header.Width; ++j){
				k = i * rowSize + j * 3;
				k1 = (15 + i) * rowSize_merged + (15 + j)* 3;
				k2 = (15 + i) * rowSize_merged + (30 + 2 * header.Width - 1 - j) * 3;
				imageData_merged[k1] = imageData[k];
				imageData_merged[k2] = imageData[k];
				imageData_merged[k1+1] = imageData[k+1];
				imageData_merged[k2+1] = imageData[k+1];
		...
		for(i=0; i<height_merged; i++){
			for(j=0; j<header.Width; ++j){
				if(i<=15 || i>=(header.Height + 15) || j<=15 || (j>= (header.Width + 15) && j<= header.Width + 30) || j>=(2*header.Width + 30)){
					k = i * rowSize_merged + j * 3;
					imageData_merged[k] = frame_B;
					imageData_merged[k+1] = frame_G;
					imageData_merged[k+1] = frame_R;
	...
	*/
	...
	/*****  Complete this code segment.  *****/
	/*****************************************/  
	if(1) fptr = fopen("landscape", "wb");
	else fptr = fopen("portrait", "wb");
