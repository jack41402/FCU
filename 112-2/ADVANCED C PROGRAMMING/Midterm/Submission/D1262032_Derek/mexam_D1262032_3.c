...
int main() {
	...
	*/
	...
	if(orientation==1){
		...
		header.Width=header.Width+2*frame_width;
		header.Height=2*header.Height+3*frame_width;
		fillings=(4-(header.Width*3%4))%4;
		rowSize_merged=header.Width*3+fillings;
		header.Size=header.Size-header.ImageSize+(rowSize_merged*header.Height);
		header.ImageSize=rowSize_merged*header.Height;
		imageData_merged=(unsigned char*)malloc(header.ImageSize);
		for(i=0;i<height;i++){
			for(j=0;j<width;j++)	{
				k=i*rowSize+j*3;
				k1=(i+2*frame_width+height)*rowSize_merged+(frame_width+j)*3;
				k2=(height-i-1+frame_width)*rowSize_merged+(frame_width+j)*3;
		...
		for(i=0;i<header.Height;i++){
			for(j=0;j<header.Width;j++){
				if((i>=0 && i<=(frame_width-1))||(i>=(frame_width+height)&&i<=(2*frame_width+height-1))||(i>=(2*frame_width+2*height)&&i<=(3*frame_width+2*height-1))||(j>=0&&j<=(frame_width-1))||(j>=(frame_width+width)&&j<=(2*frame_width+width-1)))	{
					k=i*rowSize_merged+j*3;
					imageData_merged[k]=240;
					imageData_merged[k+1]=240;
					imageData_merged[k+2]=20;
				}
		...
		width=header.Width;
		height=header.Height;
		header.Width=2*header.Width+3*frame_width;
		header.Height=header.Height+2*frame_width;
		fillings=(4-(header.Width*3%4))%4;
		rowSize_merged=header.Width*3+fillings;
		header.Size=header.Size-header.ImageSize+(rowSize_merged*header.Height);
		header.ImageSize=rowSize_merged*header.Height;
		...
		for(i=0;i<height;i++){
			for(j=0;j<width;j++)	{
				k=i*rowSize+j*3;
				k1=(i+frame_width)*rowSize_merged+(frame_width+j)*3;
				k2=(i+frame_width)*rowSize_merged+(header.Width-frame_width-j-1)*3;
				imageData_merged[k1]=imageData[k];
				imageData_merged[k1+1]=imageData[k+1];
				imageData_merged[k1+2]=imageData[k+2];
				imageData_merged[k2]=imageData[k];
		...
		for(i=0;i<header.Height;i++){
			for(j=0;j<header.Width;j++){
				if((i>=0 && i<=(frame_width-1))||(i>=(frame_width+height)&&i<=(2*frame_width+height-1))||(j>=0&&j<=(frame_width-1))||(j>=(frame_width+width)&&j<=(2*frame_width+width-1))||(j>=(2*frame_width+2*width)&&j<=(3*frame_width+2*width-1)))	{
					k=i*rowSize_merged+j*3;
					imageData_merged[k]=240;
					imageData_merged[k+1]=240;
					imageData_merged[k+2]=20;
	...
	if(orientation==1){strcpy(fileName,"landscape.bmp");  }
	else strcpy(fileName,"portrait.bmp"); 
	...
	fwrite(imageData_merged, 1, header.ImageSize, fptr);
