...
char *strncpy_ite(char *str1, const char *str2, size_t n) {
  ...
  // Copy str2 from the first character to the last one until reaching end-of-string.
  for (i=0; str2[i]!='\0' && i<n; i++) str1[i] = str2[i];
  str1[i] = '\0'; // Place end-of-string symbol (the null character) in str1.
  return str1; // Return the starting address of str1.
}
