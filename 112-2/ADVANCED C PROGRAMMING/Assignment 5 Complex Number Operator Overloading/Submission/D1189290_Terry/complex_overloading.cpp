...
ostream& operator<<(ostream &output, const Complex &r) {
	if (r.im==0.0) output << r.re;
  else if (r.im>0.0) {
	  if (r.re==0.0) output << r.im << "i";
	  else output << r.re << "+" << r.im << "i";
  }
  else {
	  if (r.re==0.0) output << r.im << "i";
	  else output << r.re << r.im << "i";
...
Complex::Complex(const double r, const double i) {
	re = r; // Set the real part.
	im = i; // Set the imaginary part.
}

// Copy constructor.
Complex::Complex(const Complex &r){
  re = r.re; // Set the real part.
  im = r.im; // Set the imaginary part.
}
