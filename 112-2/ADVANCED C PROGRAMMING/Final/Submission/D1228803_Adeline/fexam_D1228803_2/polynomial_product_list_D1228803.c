//polynomial_product_list_D1228803.c
...
Degree replaceTerm(Polynomial *P, Degree deg, Coeff coeff)
{
	...
	Term current((char *) "Current");
	while(!this->isEmpty())
	{
		current.getCoeff(this->searchTerm());
	}
	while(!current.isEmpty())
	{
		this->getCoeff(current.searchTerm());
	}
}
