...
int main(void) {
	...
	do {
		...
		*/ 
		int siden, intn, n, j, k;
		

		siden = thick*(length + (length-1)*2 + length-2) - (thick-1)*4;
		
		for(k=0; k<length; k++){
			intn = intn + (2*k-1);
		}		
		
		cout<<"The number of side characters"<<" ' "<<sChar<<" ': "<<siden;
		cout<<"\n\n";
		
		cout<<"The number of interior characters"<<" ' "<<iChar<<" ': "<<intn;
		cout<<"\n\n";
		
		cout<<"Print the rhombus: ";
		cout<<"\n";
		
		for(n=length+5; n>5; n--){
			cout<<setw(n)<<setfill(' ')<<" "<< sChar;
			if(i!=0){
				for(j=0; j<i*2+1; j++){
					cout<<setw(j)<<setfill(iChar);
				}
				cout<<sChar;
			}
			i++;
		...
		for(i=0; i<length-1; i++){
			cout<<setw(i+8)<<setfill(' ')<<sChar;
			if(j!=length-1){
				for(n=0; n<2*(length-j)-3; n++){
					cout<<setw(n)<<setfill(iChar);
				}	
				if(j!=length-2){
					cout<<sChar;
				}
