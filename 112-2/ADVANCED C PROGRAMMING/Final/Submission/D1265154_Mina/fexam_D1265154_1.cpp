...
int main(void) {
	...
	do {
		...
		cout << "The number of side characters "  << "'"  << sChar  << "'"  << " : "<< 4*length-4 << endl << endl;
		cout << "The number of interior characters "  << "'"  << iChar  << "'"  << " : "<<  ((length-2)*(length-2) + (length*4+1))<< endl << endl;
...
//				cout << setw(10+length-i+1) << setfill(' ') <<" " << sChar<< setw(2*i-2) << setfill(sChar)<< sChar << endl; 
