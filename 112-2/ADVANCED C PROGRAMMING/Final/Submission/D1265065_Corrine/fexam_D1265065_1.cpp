...
int main(void) {
	...
	do {
		...
		*/ 
		cout << endl << "The number of side charachters '" << sChar << "': ";
		for (i = length; i > length-thick; i--){
			side = (4 * (i - 1)) + side;
		}
		cout << side << endl;
		
		cout << endl << "The number of interior charachters '" << iChar << "': ";
		...
		for (i = 0; i < length; i++){
			...
			else cout << setw(i*2-1) << setfill(sChar) << "";
			cout << endl;
		}
		for (i = length; i > 0; i--){
			cout << setw(5) << setfill(' ') << "";
