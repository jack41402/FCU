...
#include <math.h>
#include <time.h>
#include "polynomial_product_list_D1228817.h"
...
srand(time(NULL));	
	...
	for(i=0; i<=deg; i++){
		if(i!=deg){
			coeff=rand()%2;
			if(coeff==1){
				coeff=rand()%10000/1000.0+1;
			}
			
			if(coeff!=0){
				replaceTerm(P, deg, coeff);
			}
		}
		else{
			coeff=rand()%10000/1000.0+1;
...
int main(void) {
	...
	printf("Evaluation of Polynomial P(a): %E", evalPolynomial(P, a));
	
	// Evaluate polynomial Q(a).
	// Print the vlaue of Q(a).
	printf("Evaluation of Polynomial Q(a): %E", evalPolynomial(Q, a));
