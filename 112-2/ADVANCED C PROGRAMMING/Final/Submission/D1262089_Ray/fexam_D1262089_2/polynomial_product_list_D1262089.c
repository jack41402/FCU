...
Polynomial productPolynomial(Polynomial P, Polynomial Q) {
	Term pcurrent=P; // Term pointer.
	Term qcurrent=Q;
	int i;
	int j;
	int pcoeff;qcoeff;
	for(i=1;i<=getTerm(Polynomial P);i++){		
		coeff=getCoeff(Polynomial P, Degree deg);
		for(j=1;i<=getTerm(Polynomial Q);j++){			
			qcurrent=qcurrent->next;
		}
		pcurrent = pcurrent->next; // Next term.		
	}	


	return ; // The number of terms of the the polynomial.
