...
int main(int argc, char *argv[]) {
  ...
  unsigned char *merged_imageData;//Pointer: merged image pixel dara.
  char file_name[40];
  ...
  do{
    printf("Enter the size of frame in pixel (between 4 and 20): ");// Input size of frame
  	scanf("%d", &Size_F);
  }while(Size_F<4 || Size_F>20);
...
/* ==================================================================*/
  ...
  // Update the width and length of the image pixels, half size of the input image.
  // If the input image has odd width or length, take the ceiling.
  merged_header.Width = io_header.Width + 3 * Size_F; // Image width in pixel.
  merged_header.Height = io_header.Height + 3 * Size_F; // Image height in pixel.
 ...
 // Peform the merge operation.
  ...
  for (i = 0; i < reduced_header.Height; i++) { // Go through all rows of the reduced image.
    for (j = 0; j < reduced_header.Width; j++) { // Go through all pixels in each row of the reduced image.
      ...
      k_1 = (i + io_header.Height / 2 + 2 * Size_F) * rowSize_merged + (io_header.Width - 1 - j + 2 * Size_F) * 3 ; // 1st quadrant.
      k_2 = (i + io_header.Height / 2 + 2 * Size_F) * rowSize_merged + (j + Size_F) * 3; // 2nd quadrant. 
      k_3 = (reduced_header.Height - 1 - i + Size_F) * rowSize_merged + (j + Size_F) * 3; // 3rd quadrant. 
      k_4 = (reduced_header.Height - 1 - i + Size_F) * rowSize_merged + (io_header.Width - 1 - j + 2 * Size_F) * 3 ; // 4th quadrant.
