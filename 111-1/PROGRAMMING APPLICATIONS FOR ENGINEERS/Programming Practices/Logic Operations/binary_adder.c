...
int adder(unsigned int X, unsigned int Y, int *c) {
  unsigned int S = 0; // At the end S=X+Y and its value is returned.
...
int main(void) {
  ...
  while (1){ // Continue to process the adder until both input are 0's.  
    ...
    printf("X = %-10u  Binary value: ", X); // Output decimal value of X with left alignment.
    printBinary(X); // Output binary value of X.
    printf("\n"); // Output a new line.
    printf("Y = %-10u  Binary value: ", Y); // Output decimal value of Y with left alignment.
    printBinary(Y); // Output binary value of Y.
    printf("\n"); // Output a new line.
    printf("S = %-10u  Binary value: ", S); // Output decimal value of the sum S with left alignment.
    printBinary(S); // Output binary value of S.
    printf("\n"); // Output a new line.
  
    // Print the confirming message. The else case should not be executed.
    if (X+Y==S) printf("Correct! Adder operation test: %u + %u = %u\n", X, Y, S);
    else printf("Incorrect! Adder operation test: %u + %u != %u\n", X, Y, S);
  
    // Print the overflow message.
    if (c!=0) printf("**** The addition operation is overflow.\n"); // If the final carry_out is 1, overflow occurs.
