...
/* ==================================================================*/
  ...
  for (i = 0; i < reduced_header.Height; i++) { // Go through all rows of the reduced image.
    for (j = 0; j < reduced_header.Width; j++) { // Go through all pixels in each row of the reduced image.
      ...
      k_1 = (i + io_header.Height / 2) * rowSize + (io_header.Width - 1 - j) * 3; // 1st quadrant.
      k_2 = (i + io_header.Height / 2) * rowSize + j * 3; // 2nd quadrant. 
      k_3 = (reduced_header.Height - 1 - i) * rowSize + j * 3; // 3rd quadrant. 
      k_4 = (reduced_header.Height - 1 - i) * rowSize + (io_header.Width - 1 - j) * 3; // 4th quadrant.
  ...
  write_image_file(file_name, io_header, io_palette, io_imageData);
