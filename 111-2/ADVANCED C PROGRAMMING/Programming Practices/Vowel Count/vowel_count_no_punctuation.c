...
int main(void) {
	...
	while (fscanf(in_file, "%s", word)==1) { // Input a string until no more in the standard input stream.
		j = 0;
		for (i=0; i<strlen(word); i++) { // Check every character is word.
			...
			// The condition below can be written as "(!ispunct(word[i]))". 
			if (isalnum(word[i])) { // If the current character is a letter or a digit, copy the letter.
				word[j] = word[i];
				j = j + 1;
			} // If the condition is false, j is not incremented.
		}
		word[j] = '\0'; // Insert end of string at the end of j.
		if (j>0) {
