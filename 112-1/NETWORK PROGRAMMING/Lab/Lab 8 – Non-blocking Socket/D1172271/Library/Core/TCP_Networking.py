...
class SimpleServer:
	def __init__(self, bind_interface, bind_port, backlog, server_privatekey=None, server_certificate=None, client_certificate=None):
...
class SimpleClient:
	def __init__(self, server_address, server_port, TLS=None, client_privatekey=None, client_certificate=None, server_certificate=None):
		self.TLS = False
		self.mTLS = False
...
class ConcurrentServer:
	...
	def __client_wrapper(self, connection, addr):
		try:
			...
			self.logger.debug("Client connection wrapper invoking the transmission loop for \'{host}:{port}\'".format(host=addr[0], port=addr[1]))
			self.transmission_loop(connection)

		except Exception as ex:
			self.logger.error("[ERROR] Got an unexpected exception in thread \'{thread}\' for host \'{host}:{port}\' !!".format(thread=threading.current_thread().name,
																											 host=addr[0],
																											 port=addr[1]))
			...
			with self.tracking_lock: # prevent race condition on client connection tracking
				...
				self.logger.debug("Transmission with host \'{host}:{port}\' ended elegently.".format(host=addr[0], port=addr[1]))
	...
	def transmission_loop(self, connection):
		...
		self.logger.warning("Please subclass the ConcurrentServer Class and override the transmission_loop(connection) for custom implementation !")
		
		# warn them by sending a dummy packet to client
		self.send(connection, "Warning !! Default Transmission Loop !!", datatype=Utils.EncodeType.STR)
		return
