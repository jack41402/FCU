...
int main(void) {
	...
	unsigned int value=0;
		...
		//     than or equal to 4,294,967,295. Print the decimal valuf n without leading zeros. 
	for(i=0;i<count;++i){
	
	value*=16;
	value += num[i]>9?num[i]+10-'A':num[i]+10-'0';
	num[i] = '\0';
	return value;
	...
	printf("The input hexadecimal numeral %s is of the decimal value: %s",num,value);
	power=pow(2,32);
	pow(o()
	for(i=0;i<count;++i){
	 value= value+num[i]*2-'0';
		
	}
	printf("Binary numeral: %s",power);
