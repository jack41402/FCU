...
int main(void) {
	...
	do {
		...
		for(i=0;i<n;i++){
			non_zero_coeff=0;
			for(j=0;j<n;j++){
				if(A[i][j]==0)non_zero_coeff+=1;
				if(non_zero_coeff==n)printf("<<<<Coefficients of Equation %d are all zeros. Stop!\n\n",i);continue;
			}
		}  
		for(j=0;j<n;j++){
			non_zero_coeff=0;
			for(i=0;i<n;i++){
				if(A[i][j]==0)non_zero_coeff+=1;
				if(non_zero_coeff==n)printf("<<<<Coefficients of Variable X%d are all zeros. Stop!\n\n",j);continue;
			}
		}
		...
		for(i=0;i<n;i++){
			...
			for(j=0;j<n;j++){
				if(j==0){
					if(A[i][j]==0)printf("        ");
					else if(A[i][j]==1)printf("    X0");
					else if(A[i][j]==-1)printf("   -X0");
					else printf("%3d X0",A[i][j]);
				}else{
					if(A[i][j]==0)printf("        ");
					else if(A[i][j]==1)printf("  + X%d",j);
					else if(A[i][j]==-1)printf("  - X%d",j);
					else if(A[i][j]<-1)printf(" -%3d X%d",-A[i][j],j);
					else if(A[i][j]>1)printf(" +%3d X%d",A[i][j],j);
				}		
			}
			printf(" = %3d",C[i]);
