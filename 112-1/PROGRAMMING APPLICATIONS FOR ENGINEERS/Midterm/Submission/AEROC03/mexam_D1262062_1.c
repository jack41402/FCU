		...
		printf("\n<<<<The linear equation system of rank %d is:\n", n);
			...
			for(k=0; k<n; ++k)
			{
				if(k==first&&k==0)
				{
					...
					else if(A[i][k]==-1)printf("-    X%d", k);
					else printf("  %2d X%d",A[i][k], k);
			
				}
				else if(k==first&&k!=0)
				{
					for(b=0; b<(k-1); ++b)printf("       ");
					if(A[i][k]==1)printf("     X%d", k);
					else if(A[i][k]==-1)printf("-    X%d", k);
					else printf("  %2d X%d",A[i][k], k);
				}
				else if(k<first)printf("");
				else
				{
					if(A[i][k]==0)printf("       ");
					else if(A[i][k]==1)printf("+    X%d", k);
					else if(A[i][k]==-1)printf("-    X%d", k);
					else if(A[i][k]>0)printf("+ %2d X%d",A[i][k], k);
					else 
					{
						A[i][k] = -A[i][k];
						printf("- %2d X%d",A[i][k], k);
					}
