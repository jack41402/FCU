...
int main(void) {
	...
	do {
		...
		for(i=0;i<n;i++){
			...
			if(v[i]==n){
				printf("<<<< Coefficients of Equation %d are all zeros. Stop!\n",i);
			}
		}
		for(i=0;i<n;i++){
			for(j=0;j<n;j++){
		...
		if(verify0==n&&verify1==n){
			printf("<<<< The linear equation system of rank %d is:\n",n);
			for(i=0;i<n;i++){
				printf("          ");
				for(j=0;j<n;j++){
					if(j==0){
						if(A[i][j]!=1&&A[i][j]!=-1&&A[i][j]!=0) printf(" %3d X%d ",A[i][j],j);
