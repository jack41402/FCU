...
int main(void) {
	...
	int a, i, j, nm,cf,cnst,d=0; // Loop variables.
	char X,m;
	...
	do {
		...
		for (a=0;a<n;a++){
			printf("Enter %d coefficients and a constant for Equation %d (between -99 and 99)\n",n,a);
			for(i=0;i<n;i++){
				scanf("%d",&cf);
				 A[a][i]=cf;
				 if(cf==0){
				 ...
				 if(d==n){
				 	printf("Coefficients of equation %d are all zero",i);
				 	break;
				 }
			}
			scanf("%d",&cnst);
		...
		printf("The linear equation of system rank %d is:\n",n);
		for(i=0;i<n;i++){
			for(a=0;a<n;a++){
				if (A[i][a]>0){
					m='+';
				}
				printf("%c %d X%d ",m,A[i][a],a);	
			}
			printf("=%d\n",C[i]);
		}
