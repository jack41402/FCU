...
int main(void) 
{
	...
	do {
		...
		if (n==0)
		{
			printf("n could not be 0");
			break;
		}
		 // Program terminates, if n is 0.
		 
		 
		 for(i=0; i<n; ++i)
		{
			printf(">>>>>Enter %d coefficient and a constant for Equation %d(between -99 and 99): ", n, n);
			scanf("%d %d %d %d %d", &A[9][9], &A[9][9], &A[9][9], &A[9][9], C[9]);
		}
		...
		if(non_zero_coeff!=0)
		{
			if(non_zero_coeff!=1&&non_zero_coeff!=-1)
			{
				printf("%d X");
				scanf("%d", coefficient);
				printf("%d", A[9][9]);
			}
			else if(non_zero_coeff=1)
			{
				printf("X%d", n);
			}
			else
