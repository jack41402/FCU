...
double right_Riemann_sum(double r, double s, double (*f)(double), double (*g)(double)) {
	...
	area=-1;
	last=-2;
	while (area-last>epsilon){
		interval=(a-0)/T;
		last=area;
		area=0;
		point=c;
		for(i=0;i<T;i++){		    			
			area+=(f1(point)-f4(point))*interval;
			point+=interval;			
		}
	T*=2;	
	}
...
int main(void) {
	...
	while (area-last>epsilon){
		...
		point=c;
		for(i=0;i<T;i++){		    			
			area+=(f2(point)-f3(point))*interval;
			point+=interval;			
		}
    printf("Number of intervals: %d, interval size:%d, area:%.6lf\n",T,interval,area);
	T*=2;	
	}
	printf("Number of intervals:%d",T/2);
	double area1=area;
	...
	printf(">>>>Area of the triangle BCD:%d",area1);
	printf(">>>>Area of the triangle BAD:%d",area2);
