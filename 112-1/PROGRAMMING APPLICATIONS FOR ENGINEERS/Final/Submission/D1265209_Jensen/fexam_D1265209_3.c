...
double right_Riemann_sum(double r, double s, double (*f)(double), double (*g)(double)) {
	...
	while(fabs(area-last)>epsilon){
		...
		for(i=0; i<T; i++){
			...
			point += interval;
			printf("Number of interval: %d, ", T);
			printf("interval size: %d, ", fabs(r-s) / T);
			printf("area: %d\n", fabs((*f)(point) - (*g)(point)) * interval);
		}
		T = T * 2;
	}
	return area;
...
int main(void) {
	...
	right_Riemann_sum(a, 0, *f1, *f4);
	printf("The number of interval: %d", T);
	areaBAD = right_Riemann_sum(a, 0, *f1, *f4);
	printf("**** Compute the area of triangle BAD. \n");
	right_Riemann_sum(0, c, *f2, *f3);
	printf("The number of interval: %d", T);
	areaBCD = right_Riemann_sum(0, c, *f2, *f3);
	area = areaBAD + areaBCD;
	// e. Print the area of triangle BCD, the area of triangle BAD, and the area of quadrilateral ABCD.
	printf(">>>> Area of triangle BCD: %d\n", areaBCD);
	printf(">>>> Area of triangle BAD: %d\n", areaBAD);
	printf(">>>> Area of quadrilateral ABCD: %d\n", area);
  	
	// f. Verify the result with the area formula 1/2|(a-c)(b-d)|.
	printf(">>>> The result of 1/2|(a-c)(b-d)| is: %d\n", (1/2) * fabs((a-c) * (b-d)));
	printf(">>>> The error is: %d", area - ((1/2) * fabs((a-c) * (b-d))));
