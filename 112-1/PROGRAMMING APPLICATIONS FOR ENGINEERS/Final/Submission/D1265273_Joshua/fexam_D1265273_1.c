...
int main(void) {
	...
	for (i=0; i<m; i++)
	{
		for (j=max(0,i-r-1); j<=min(s+i, n); j++)
		{
			A[i][j]= rand()%100;
		}
	} 
	...
	printf("MatrixA, %dX%d with lower bandwidth %d and upper bandwidth %d:\n", m, n, r,s);
	for (i=0; i<m; i++)
		{
			for (j=0; j<n; j++)
			{
		...
		printf("\nMatrixB, %dX%d with lower bandwidth %d and upper bandwidth %d:\n", n, m, s,r);
		for (i=0; i<n; i++)
		{
			for (j=0; j<m; j++)
			{
