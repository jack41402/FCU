...
double right_Riemann_sum(double r, double s, double (*f)(double), double (*g)(double)) {
	...
	double area=0; // Area of the covered region, left Riemann sum of the current iteration.
	double last=1; // Left Riemann sum of the previous iteration.
	int T=1;
	int i; // Loop variable.
	while(fabs(area-last)>pow(10, -6)){
		last=area;
		area=0;
		interval=(fabs(r-s))/T;
		point=r+interval/2;
		for(i=0; i<T; i++){
			area+=(f(point+interval*i)-g(point+interval*i))*interval;
		}
		printf("The number of intervals: %d, interval size:%8.6f, area: %8.6f\n", T, interval, area);
		T*=2;
	}
   return area;
}
...
int main(void) {
	...
	printf("**** Compute area of triangle BCD.\n");
	areaBCD=right_Riemann_sum(c, 0, f2, f3);
	
	printf("\n\n**** Compute area of triangle BAD.\n");
	areaBAD=right_Riemann_sum(0, a, f1, f4);
	printf("\n\n>>>>Area of triangle BCD: %8.6f\n", areaBCD);
	printf("\n\n>>>>Area of triangle BAD: %8.6f\n", areaBAD);
	area=areaBCD+areaBAD;
	printf(">>>>Area of the quadrilateral ABCD is: %8.6f\n", area);
	printf("The result of 1/2|(a-c)(b-d)| is %8.6f\n", 0.5*fabs((a-c)*(b-d)));
	printf("The error is: %8.6f", area-0.5*fabs((a-c)*(b-d)));
