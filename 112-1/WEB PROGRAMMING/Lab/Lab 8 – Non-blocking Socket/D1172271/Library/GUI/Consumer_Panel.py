...
class ClientPanel(QWidget): 
	...
	def init_socket(self):
		try:
			...
			# try to request a number from the server
			self.client.send("[GET-REQUEST]", datatype=Utils.EncodeType.STR)
			self.Text_Browser.append("[INFO] Sending the command \'[GET-REQUEST]\' to server......\n")
			
			# start the transmission loop
			self.start_transmission()

		except OSError as ex:
			self.pop_messagebox("[ERROR]", "Socket error on initializing client TCP socket", str(ex))
			sys.exit(-1)

		except Exception as ex:
			self.pop_messagebox("[ERROR]", "Unexpected error on initializing client TCP socket", str(ex))
			sys.exit(-1)
	...
	def start_transmission(self):
		...
		self.thread_worker = TransmissionHdlr(self.client)
...
class TransmissionHdlr(QThread):
	...
	def __init__(self, hdlr, timeout=RECEIVE_TIMEOUT_SECONDS, max_retry=MAX_TIMEOUT_RETRY):
		super().__init__()
		self.client = hdlr
	...
	def _process_integer_data(self, data):
		...
		self.update_signal.emit("[INFO] Sending next \'[GET-REQUEST]\' to server")
		self.client.send("[GET-REQUEST]", datatype=Utils.EncodeType.STR)
	...
	def _process_string_data(self, data):
		...
		if (data == self.READY_SIGNAL):
			...
			self.update_signal.emit("[INFO] Sending \'[GET-REQUEST]\' to server.......")
			self.client.send("[GET-REQUEST]", datatype=Utils.EncodeType.STR)
			connection_status = True

		elif (data == self.NOTREADY_SIGNAL):
		...
		elif (data == self.KEEPALIVE_SIGNAL):
			print("[WARNING] Server not ready, continue pending state !")
			self.update_signal.emit("[INFO] Sending \'[PENDING]\' to server......")
			self.client.send("[PENDING]", datatype=Utils.EncodeType.STR)
			connection_status = True

		elif (data == self.TERMINATION_SIGNAL):
			self.update_signal.emit("[INFO] Terminate signal recived, closing connection......\n")
			return False
